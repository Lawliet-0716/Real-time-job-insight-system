...
export async function compareSkills(resumeSkills, targetRole = "") {
  try {
    ...
    const skillTrends = await SkillTrend.find().select(
    ...
    const roleEntry = Object.entries(roleKeywords).find(
      ([role]) =>
        normalizedRole.includes(role) || role.includes(normalizedRole),
    );

    const keywords = roleEntry ? roleEntry[1] : [normalizedRole];

    // Filter jobs relevant to target role
    // ---------------------------------------

    const roleRelevantSkills = skillTrends.filter((item) => {
      const jobTitles = Array.isArray(item.jobTitles) ? item.jobTitles : [];
    ...
    let marketSkills =
      roleRelevantSkills.length >= 5 ? roleRelevantSkills : skillTrends;
    ...
    marketSkills.forEach((item) => {
      ...
      if (!uniqueSkills.has(normalizedSkill)) {
        uniqueSkills.set(normalizedSkill, item);
      }
    });

    marketSkills = [...uniqueSkills.values()];

    // ---------------------------------------
    // Remove zero-demand skills
    // ---------------------------------------

    const skillsWithDemand = marketSkills.filter((item) => {
      const demandCount = Number(item.demandCount) || 0;

      const percentage = Number(item.percentage) || 0;

      return demandCount > 0 || percentage > 0;
    });

    if (skillsWithDemand.length >= 5) {
      marketSkills = skillsWithDemand;
    }
    ...
    marketSkills.sort((a, b) => {
      const demandA = Number(a.demandCount) || 0;

      const demandB = Number(b.demandCount) || 0;

      const percentageA = Number(a.percentage) || 0;

      const percentageB = Number(b.percentage) || 0;

      const scoreA = demandA + percentageA;

      const scoreB = demandB + percentageB;

      return scoreB - scoreA;
    });

    // ---------------------------------------
    // Score against the top requirements for this role. The resume extractor
    // returns up to 25 skills, so this avoids penalizing a strong resume with
    // a long list of unrelated market requirements.
    // ---------------------------------------

    marketSkills = marketSkills.slice(0, 25);
    ...
    const matchPercentage =
      marketSkills.length === 0
        ? 0
        : Math.round((matchingSkills.length / marketSkills.length) * 100);
    ...
    marketSkills.forEach((item) => {
      ...
      const weight = demandCount + percentage;
    ...
    weightedMatchPercentage = Math.min(
      100,
      Math.max(0, weightedMatchPercentage),
    );
    ...
    const atsScore = weightedMatchPercentage;
    ...
    console.log("Market Skills:", marketSkills.length);
    ...
    console.log("Weighted Match:", weightedMatchPercentage);
