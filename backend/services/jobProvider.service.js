  ...
  try {
    const response = await axios.get(JSEARCH_URL, {
      params: {
        ...
        language: "en",
        num_pages: numPages,
        ...(options.datePosted ? { date_posted: options.datePosted } : {}),
      },
      headers: {
    ...
    await Promise.all(
      mappedJobs.map(async (job) => {
        job.skills = await extractJobSkills(job.title, job.description);
      }),
    );
    ...
    const newJobs = mappedJobs.filter(
      (job) => !existingSet.has(`${job.source}-${job.externalJobId}`),
    );
    ...
    await Promise.all(insertedJobs.map(updateSkillTrends));
