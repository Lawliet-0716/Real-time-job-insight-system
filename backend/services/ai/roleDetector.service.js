...
`;
    ...
    if (!content) {
      console.error("❌ AI returned empty role response");
    ...
    if (startIndex === -1 || endIndex === -1 || endIndex <= startIndex) {
      console.error("❌ Role JSON not found");
    ...
    try {
      ...
      console.error("❌ Could not parse role JSON");
    ...
    // Validate role
    const targetRole =
      typeof result.targetRole === "string" ? result.targetRole.trim() : "";

    // Reject invalid placeholder values
    const invalidRoles = ["", "<role>", "role", "n/a", "na", "unknown"];

    if (invalidRoles.includes(targetRole.toLowerCase())) {
      console.error("❌ Invalid target role:", targetRole);

      return "";
    }

    console.log("✅ Detected Target Role:", targetRole);
